...
void loop() {
    ...
    if (update_display_window) {
        ...
        draw_cursor();
        int counter;
        for (counter = 0;counter< (path_len -1);counter ++){
            uint16_t firstx = longitude_to_x(current_map_num,waypoints[counter].lon);
            uint16_t firsty = latitude_to_y(current_map_num,waypoints[counter].lat);
            uint16_t secondx = longitude_to_x(current_map_num,waypoints[counter+1].lon);
            uint16_t secondy = latitude_to_y(current_map_num,waypoints[counter+1].lat);
            tft.drawLine(firstx,firsty,secondx,secondy,0xF800);
        }
