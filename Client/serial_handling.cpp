...
int srv_get_pathlen(LonLat32 start, LonLat32 end) {
    ...
    Serial.print(buffer);
    char split_buffer[100];

    // read the incoming byte:
    serial_readline(buffer, 100);

    string_read_field(buffer, 0, split_buffer, 100, " ");
...
int srv_get_waypoints(LonLat32* waypoints, int path_len) {
    for (int i=0; i<path_len; ++i) {
        char waypoint_buffer[100];
        char split_waypoint_buffer[100];
        Serial.print("A\n");
        serial_readline(waypoint_buffer, 100);
        string_read_field(waypoint_buffer, 0, split_waypoint_buffer, 100, " ");
        if (split_waypoint_buffer[0] == 'W'){
            waypoints[i] = LonLat32(split_waypoint_buffer[1],split_waypoint_buffer[2]);
        }
        else {
            return -1;
        }
